class vExcept(Exception):
    ...
    def __init__(self, errcode, message="Internal error"):
        ...
        match errcode:
            ...
            # Users (100-199)
            case 108:
                self.message = f"User name ({message}) does not exist"
            case 109:
                self.message = "Invalid username or password"
            case 110:
                self.message = f"User name ({message}) already exist"
            # Tables (200-279)
            case 209:
            ...
            # Sequences (280-299)
            case 280:
                self.message = f"Sequence ({message}) does not exists"
            case 281:
                self.message = f"Sequence is not unique in query ({message})"
            # Columns (300-379)
            case 310:
            ...
            # Sequence (380-399)
            case 380:
                self.message = f"Sequence ({message}) already exists"
            case 381:
                self.message = f"Sequence ({message}) does not exists"
            ...
            case 762:
                self.message = f'Syntax error: invalid sequence name "{message}"'
            case 763:
                self.message = f'Syntax error: invalid object name "{message}"'
