...
try:
    ...
    try:
        __id_db = open(file=parameters_file, mode="tr", encoding="utf-8")
    ...
    password = "restopwd"
    database = "db"
    query = "select p.id, p.name, autre.seq01.currval ant_val, autre.seq01.nextval indice, resto.seq01.nextval indice2 from resto.plats p order by p.id"
    bind = {}
    ...
    print(tabulate(cursor.fetchall(), headers=entete, tablefmt="grid"))
