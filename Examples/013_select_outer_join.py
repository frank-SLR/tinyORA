...
try:
    ...
    password = 'restopwd'
    database = 'db'
    query = ("select l.id, l.name, p.id, p.name, trunc(3.14156, 1) PI_TRUNC, acos(0.5) AC , asin(0.5) AS , atan(0.5) AT , atan2(1, 2) AT2, cos(1) cos_1, sin(1) sin_1, tan(1) tan_1, exp(1) exp_1, ln(1) ln_1, log(1) log_1 "
             + "from resto.plats p "
