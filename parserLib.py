...
# functions : [fct_id, fct_name, [[table_alias, schema, table_name, col_name/value, type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE), table position, position in table, table or cursor]]]
# in : [in_id, (LIST, CURSOR), [[table_alias, schema, table_name, col_name/value, type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE), table position, position in table, table or cursor]]]
# connect : oper, value
# maths : maths_id, [[table_alias, schema, table_name, col_name/value, type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE), table position, position in table, table or cursor]], [
#                    item_id, ['META', item_id], oper, ['META', item_id]
#                 or item_id, ['TST', num_table, num_col, alias1, field1, type, schema, table_name, table or cursor], oper, ['TST', num_table, num_col, alias2, field2, type, schema, table_name, table or cursor],
#                   , type_maths(INT, FLOAT)
#                   ...]
# pipe : pipe_id, [[table_alias, schema, table_name, col_name/value, alias, type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE), table position, position in table, table or cursor]]
# bind : [[name, value] ...]
# group_by : table_alias, schema, table_name, col_name/value, alias, type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE), table position, position in table, table or cursor
# post_tasks : (TRUE, FALSE)
# post_data_model : col_id= {obj_name= {column:[table_alias, schema,table_name,col_name/value,alias,type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE),table position,position in table,table or cursor],
...
class vParser:
    ...
    def __parse_select(self, pos):
        ...
        #   3: col_name/value
        #   4: alias
        #   5: type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE)
        #   6: table position
        #   7: position in table
        #   8: table or cursor
        ...
        #   2: table_name
        #   3: col_name/value
        #   4: type(COL, INT, FLOAT, STR, HEX, DATETIME, FUNCTION, MATHS, PIPE, SEQUENCE)
        #   5: table position
        #   6: position in table
        #   7: table or cursor
        for f in range(len(self.__parsed_query["functions"])):
            for n in range(len(self.__parsed_query["functions"][f][2])):
                if (self.__parsed_query["functions"][f][2][n][4] is None) or (self.__parsed_query["functions"][f][2][n][4] == "COLUMN"):
                    ...
                    self.__parsed_query["functions"][f][2][n] = [
                        al,
                        sh,
                        tn,
                        cn,
        ...
        #   2: table_name
        #   3: col_name/value
        #   4: type(COL, INT, FLOAT, STR, HEX, DATETIME, COLUMN, FUNCTION, MATHS, PIPE, SEQUENCE)
        #   5: table position
        #   6: position in table
        #   7: table or cursor
        ...
        #   3: table_name
        #   4: col_name/value
        #   5: type(COL, INT, FLOAT, STR, HEX, DATETIME, COLUMN, FUNCTION, MATHS, PIPE, SEQUENCE)
        #   6: table position
        #   7: position in table
        #   8: table or cursor
        ...
        for f in range(len(self.__parsed_query["pipe"])):
            for n in range(len(self.__parsed_query["pipe"][f][1])):
                if self.__parsed_query["pipe"][f][1][n][5] in ["FUNCTION", "MATHS"]:
                    ...
                    self.__parsed_query["pipe"][f][1][n] = [
                        al,
                        sh,
                        tn,
                        cn,
        ...
        for gcol in range(len(self.__parsed_query["group_by"])):
            for scol in range(len(self.__parsed_query["select"])):
                ...
                if len(gcolname) == 1:
                    ...
                    if ((gcolname[0] == str(self.__parsed_query["select"][scol][2]).upper()) or (gcolname[0] == str(self.__parsed_query["select"][scol][0]).upper())) and (
                        gcolname[1] == str(self.__parsed_query["select"][scol][3]).upper()
                    ):
                        self.__parsed_query["group_by"][gcol] = self.__parsed_query["select"][scol][0:9]
                        col_found = True
                elif len(gcolname) == 3:
                    if (
                        (gcolname[0] == str(self.__parsed_query["select"][scol][1]).upper())
                        and (gcolname[1] == str(self.__parsed_query["select"][scol][2]).upper())
                        and (gcolname[2] == str(self.__parsed_query["select"][scol][3].upper()))
                    ):
                        self.__parsed_query["group_by"][gcol] = self.__parsed_query["select"][scol][0:9]
                        col_found = True
            if not col_found:
        ...
        for f in range(len(self.__parsed_query["maths"])):
            ...
            m_idx, lst_maths, v_idx, bracket = self.__compute_maths(
                self.__parsed_query["maths"][f][1],
                m_idx,
                lst_maths,
                v_idx,
                None,
    ...
    def __parse_SEL_COL(self, pos):
        ...
        while (word.upper() != "FROM") and (pos < len(self.__query)):
            ...
            if word.upper() in [",", "FROM"]:
                ...
                if word.upper() in [",", "FROM"]:
                    self.__parsed_query["select"].append(
                        [
                            None,
                            None,
                            None,
    ...
    def __parse_pipe(self, col, word, pos):
        ...
        if word == col:
            tmpP = [
                pipe_id,
                [[None, None, None, word, None, "COLUMN", None, None, None]],
            ]
        else:
            tmpP = [
                pipe_id,
    ...
    def __parse_COL_IN(self, pos):
        ...
        while word != ")":
            ...
            if self.__is_function(word.upper()):
                ...
                #     word = word[1:-1]
                self.__parsed_query["in"][in_idx][2].append(
                    [
                        None,
                        None,
                        None,
                        self.__remove_quote(str(word)),
                        "STR",
                        None,
                        None,
                        None,
        ...
        while pos < len(self.__query):
            ...
            if word == ",":
                ...
                if self.__is_function(col.upper()):
                    ...
                    if word != ",":
                        self.__parsed_query["group_by"].append(
                            [
                                None,
                                None,
                                None,
    ...
    def __getColFromTable(self, colin: str):
        ...
        if colin.upper() == "ROWNUM":
            ...
            tab_cur1 = None
            num_tab = None
        elif (tmp1[-1] in ["NEXTVAL", "CURRVAL"]) and (len(tmp1) in [2, 3]):
            fmt1 = "SEQUENCE"
            alias1 = None
            tab_cur1 = None
            num_tab = None
            ...
            alias1, col1, schema1, table_name1, tab_cur1, num_tab = (
                None,
                colin,
                None,
                None,
        ...
        if w_idx < len(parsed_where):
            ...
            if len(x) > 1:
                ...
                if self.__is_parsed_function(x[0]):
                    fmt1, alias1, col1, schema1, table_name1, tab_cur1, num_tab1 = (
                        "FUNCTION",
                        None,
                        x[0],
                        None,
                ...
                if self.__is_parsed_function(x[2]):
                    list2 = None
                    fmt2, alias2, col2, schema2, table_name2, tab_cur2, num_tab2 = (
                        "FUNCTION",
                        None,
                        x[2],
                        None,
                ...
                if (len(x) == 5) and (x[1].upper() == "BETWEEN") and (x[3].upper() == "AND"):
                    if self.__is_parsed_function(x[4]):
                        fmt3, alias3, col3, schema3, table_name3, tab_cur3, num_tab3 = (
                            "FUNCTION",
                            None,
                            x[4],
                            None,
                    ...
                    lst_where.append(
                        [
                            ...
                            x[1].upper(),
                            ["TST", None, None, None, list2, None, None, None, None],
                        ]
                    )
                else:
                    lst_where.append(
